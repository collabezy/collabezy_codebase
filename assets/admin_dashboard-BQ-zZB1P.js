import"./style-CAvnQPbK.js";import{a as e,d as t,l as n,n as r,r as i,s as a,u as o}from"./adminApi-CKNIlaRm.js";var s=document.getElementById(`total-influencers`),c=document.getElementById(`total-brands`),l=document.getElementById(`total-campaigns`),u=document.getElementById(`total-tickets`),d=document.getElementById(`influencer-tbody`),f=document.getElementById(`brand-tbody`),p=document.getElementById(`ticket-tbody`),m=document.getElementById(`instagram-verification-tbody`),h=document.getElementById(`logout-btn`);window.addEventListener(`DOMContentLoaded`,async()=>{if(!sessionStorage.getItem(`admin_token`)){window.location.href=`/admin/login.html`;return}let e=sessionStorage.getItem(`admin_user`);if(e)JSON.parse(e),document.getElementById(`admin-info`).textContent=`System Admin`;else{sessionStorage.removeItem(`admin_token`),sessionStorage.removeItem(`admin_user`),window.location.href=`/admin/login.html`;return}await g()});async function g(){try{let[t,d,f,p,m,h]=await Promise.all([n(),e(),r(),i(),o(),a()]),g=t||[],x=f||[];(d||[]).filter(e=>e.verified===!0),s.textContent=g.length,c.textContent=x.length,l.textContent=(p||[]).length,u.textContent=(m||[]).length,v(g,d||[]),y(x,p||[]),b(m||[],x,g,p||[]),_(h||[],t||[])}catch(e){console.error(`Dashboard error:`,e)}}function _(e,t){if(m.innerHTML=``,e.length===0){m.innerHTML=`<tr><td colspan="5" style="text-align: center; padding: 2rem;" class="subtitle">No pending verification requests</td></tr>`;return}e.forEach(e=>{let n=t.find(t=>t.user_id===e.user_id),r=document.createElement(`tr`);r.innerHTML=`
