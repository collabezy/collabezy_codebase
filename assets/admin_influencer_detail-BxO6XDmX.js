import"./style-CAvnQPbK.js";import{c as e,o as t}from"./adminApi-CKNIlaRm.js";var n=new URLSearchParams(window.location.search).get(`id`),r=document.getElementById(`loading`),i=document.getElementById(`content`),a=document.getElementById(`avatar`),o=document.getElementById(`inf-name`),s=document.getElementById(`inf-email`),c=document.getElementById(`inf-categories`),l=document.getElementById(`inf-bio`),u=document.getElementById(`channels-list`);window.addEventListener(`DOMContentLoaded`,async()=>{if(!sessionStorage.getItem(`admin_token`)){window.location.href=`/admin/login.html`;return}if(!n){r.textContent=`No influencer ID provided.`;return}try{let[u,f]=await Promise.all([e(n),t(n)]),p=Array.isArray(u)?u[0]:null,m=Array.isArray(f)?f:[];if(!p){r.textContent=`Influencer profile not found.`;return}o.textContent=p.name||`Anonymous`,s.textContent=p.user_id,l.textContent=p.bio||`No bio available.`,a.src=p.avatar_url||`https://via.placeholder.com/150`,p.category&&Array.isArray(p.category)&&(c.innerHTML=``,p.category.forEach(e=>{let t=document.createElement(`span`);t.className=`pill`,t.textContent=e,c.appendChild(t)})),d(m),r.style.display=`none`,i.style.display=`block`}catch(e){r.textContent=`Error loading profile: `+e.message}});function d(e){if(e.length===0){u.innerHTML=`<p class="subtitle">No social accounts linked.</p>`;return}u.innerHTML=``,e.forEach(e=>{let t=document.createElement(`div`);t.className=`glass-panel`,t.style.padding=`1.5rem`,t.innerHTML=`
